...
func TestGetTopologyHints(t *testing.T) {
	...
	for _, tc := range tcases {
		...
		hints := m.GetTopologyHints(tCtx.Logger(), tc.pod, &tc.pod.Spec.Containers[0], lifecycle.AddOperation)
...
func TestGetPodTopologyHints(t *testing.T) {
	...
	for _, tc := range tcases {
		...
		hints := m.GetPodTopologyHints(tCtx.Logger(), tc.pod, lifecycle.AddOperation)
