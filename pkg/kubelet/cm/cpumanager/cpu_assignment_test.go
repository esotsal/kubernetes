...
func TestTakeByTopologyNUMAPackedForResize(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			result, err := takeByTopologyNUMAPackedForResize(logger, tc.topo, tc.availableCPUs, tc.numCPUs, strategy, tc.opts.PreferAlignByUncoreCacheOption, tc.reusableCPUs, cpuset.New())
...
func TestTakeByTopologyNUMADistributedForResize(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			result, err := takeByTopologyNUMADistributedForResize(logger, tc.topo, tc.availableCPUs, tc.numCPUs, tc.cpuGroupSize, CPUSortingStrategyPacked, tc.reusableCPUs, cpuset.New())
			if err != nil {
				if tc.expErr == "" {
					t.Errorf("unexpected error [%v]", err)
				}
